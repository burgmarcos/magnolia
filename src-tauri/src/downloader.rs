...
pub async fn download_model(app: AppHandle, url: String, filename: String) -> Result<(), String> {
    ...
    let res = client.get(&url).send().await.map_err(|e| format!("Download failed to start: {}", e))?;
    ...
    let mut file = File::create(&file_path).await.map_err(|e| format!("Could not create file: {}", e))?;
