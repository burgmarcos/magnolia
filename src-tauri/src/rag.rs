...
pub async fn fetch_embedding(input: &str) -> Result<Vec<f32>, String> {
    ...
    let res = client.post("http://127.0.0.1:8081/v1/embeddings")
        .json(&body)
        .send()
        .await
    ...
    let mut json: EmbeddingResponse = res.json().await.map_err(|e| format!("Parse error: {}", e))?;
...
pub async fn generate_document_embeddings(conn: &mut Connection) -> Result<usize, String> {
    ...
    {
        let mut stmt = conn.prepare("SELECT id, path FROM documents WHERE id NOT IN (SELECT document_id FROM nodes)").map_err(|e| e.to_string())?;
