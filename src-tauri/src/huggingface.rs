...
pub async fn fetch_hf_model_size(model_id: &str) -> Result<HfModelInfo, String> {
    ...
    let model_data: HfModelResponse = res.json().await.map_err(|e| format!("Failed to parse JSON: {}", e))?;
    ...
    if let Some(siblings) = model_data.siblings {
        for sib in siblings {
            if sib.rfilename.ends_with(".gguf") {
                ...
                let file_url = format!("https://huggingface.co/{}/resolve/main/{}", model_id, sib.rfilename);
