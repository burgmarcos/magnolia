...
async fn process_sse_stream(app: &AppHandle, mut stream: impl futures_util::Stream<Item = reqwest::Result<bytes::Bytes>> + Unpin) -> Result<(), String> {
                            ...
                            if let Some(delta) = choice.get("delta") {
                                if let Some(content) = delta.get("content") {
                                    if let Some(content_str) = content.as_str() {
                                        let _ = app.emit("chat-token", ChatChunk {
...
pub async fn stream_chat_cloud(app: AppHandle, provider: String, model: String, messages: Vec<ChatMessage>) -> Result<(), String> {
    ...
    let url = match provider.to_lowercase().as_str() {
        ...
        "anthropic" => {
            ...
            return Err("Anthropic native streaming not fully implemented yet, use OpenAI interface.".into());
        },
