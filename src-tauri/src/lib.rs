...
#[tauri::command]
async fn download_model_file(app: tauri::AppHandle, url: String, filename: String) -> Result<(), String> {
...
#[tauri::command]
async fn stop_local_engine(state: tauri::State<'_, llm_manager::LlmEngineState>) -> Result<(), String> {
...
#[tauri::command]
async fn stream_chat_local(app: tauri::AppHandle, messages: Vec<inference::ChatMessage>) -> Result<(), String> {
    inference::stream_chat_local(app, messages).await
}

#[tauri::command]
async fn stream_chat_cloud(app: tauri::AppHandle, provider: String, model: String, messages: Vec<inference::ChatMessage>) -> Result<(), String> {
    inference::stream_chat_cloud(app, provider, model, messages).await
}

#[tauri::command]
async fn index_local_folder(state: tauri::State<'_, DbState>, path: String) -> Result<usize, String> {
...
#[tauri::command]
async fn semantic_search(state: tauri::State<'_, DbState>, query: String, limit: usize) -> Result<Vec<rag::SearchResult>, String> {
    ...
    if let Some(ref conn) = *db_state {
        ...
        let rows = stmt.query_map(rusqlite::params![emb_json, limit as isize], |row| {
            Ok(rag::SearchResult {
...
pub fn run() {
  tauri::Builder::default()
    ...
    .manage(DbState { conn: tokio::sync::Mutex::new(None) })
