...
export function ModelsDownloader() {
  ...
  useEffect(() => {
    ...
    import('@tauri-apps/api/core').then(({ invoke }) => {
      ...
      invoke<{id: string, size_bytes: number, fit_status: string}[]>('get_all_local_models_info')
        .then(localModelsInfo => {
          const installed: ModelItem[] = localModelsInfo.map(info => {
            let fit: FitState = 'cannot-run';
            if (info.fit_status === 'Fits Perfectly') fit = 'perfect';
            else if (info.fit_status === 'Needs Offload') fit = 'offload';
            return { id: info.id, name: info.id, description: 'Local model', fit, status: 'installed' as const };
          });
          setModels(installed);
        })
        .catch(e => {
    ...
    <div className={styles.container}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <AnimatePresence>
            {showKeyInput && (
              ...
              >
                ...
                />
                <button onClick={verifyHfToken} disabled={isSaving} style={iconBtnStyle} title="Verify Token">
                  <RefreshCw size={14} className={isSaving ? 'animate-spin' : ''} />
                </button>
                <button onClick={saveHfToken} disabled={isSaving} style={iconBtnStyle} title="Save Token">
                  <ShieldCheck size={14} />
                </button>
              </motion.div>
            )}
          ...
          >
            <ExternalLink size={14} /> <span style={{ fontSize: '11px', fontWeight: 600 }}>Get Token</span>
          </button>
        </div>
      </div>
      ...
      <div className={styles.cardList}>
        {isSearching ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '60px', gap: '16px', width: '100%' }}>
            <LoadingAnimation />
            <p style={{ color: 'var(--schemes-on-surface-variant)', fontSize: '13px', fontWeight: 500 }}>Scanning Hugging Face Hub...</p>
          </div>
