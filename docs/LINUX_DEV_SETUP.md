...
This guide provides the necessary steps to set up a Linux environment for developing and testing SLAI, specifically using **WSL2** on Windows 11 with **NVIDIA GPU** acceleration.
...
Rust is the core engine for SLAI's backend.
...
If you see your GPU listed, the `llama.cpp` backend in SLAI will be able to utilize CUDA for hardware-accelerated inference.

## 6. Running SLAI in WSL2
...
> [!TIP]
> **WSLg Support**: Windows 11 supports WSLg by default. The SLAI application window will appear on your Windows desktop just like a native Windows application, but it will be running inside the Linux kernel.
...
- **Slow Compilation**: Building in WSL is significantly faster if the project files are located on the Linux filesystem (e.g., `~/projects/SLAI`) rather than a Windows mount (e.g., `/mnt/c/...`).
