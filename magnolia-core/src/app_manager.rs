...
pub async fn download_app(url: String, app_id: String) -> Result<String, String> {
    ...
    if !app_dir.exists() {
        fs::create_dir_all(&app_dir).map_err(|e| format!("Failed to create app partition directory: {}", e))?;
    }

    println!("[Magnolia AppManager] Fetching package for {} from {}", app_id, url);
    ...
    let bytes = response.bytes().await.map_err(|e| format!("Failed to stream package: {}", e))?;
    ...
    let status = Command::new("tar")
        .args(["-zxf", &target_path.to_string_lossy(), "-C", &app_dir.to_string_lossy()])
        .status()
    ...
    Ok(format!("App {} successfully provisioned to {}", app_id, APPS_ROOT))
}
...
pub fn launch_app(app_id: String) -> Result<(), String> {
    ...
    if !manifest_path.exists() {
        return Err(format!("Application '{}' is not installed correctly (Missing Manifest)", app_id));
    }
    ...
    cmd.arg("--dir").arg("/home/sandbox")
       .arg("--setenv").arg("HOME").arg("/home/sandbox");
    ...
    println!("[Magnolia] Sandboxing App [{}] -> Entry: {}", manifest.name, entry_target);
