...
pub async fn archive_app(app_id: String) -> Result<(), String> {
    ...
    if fs::metadata(&bin_path).is_ok() {
        ...
        println!("[STORAGE] {} successfully archived. Local storage freed.", app_id);
        Ok(())
    } else {
...
pub async fn get_disk_info() -> Result<Vec<DiskInfo>, String> {
    ...
    let output = Command::new("lsblk")
        .args(["-J", "-b", "-o", "NAME,LABEL,MOUNTPOINT,SIZE,FSSIZE,FSTYPE,RM"])
        .output()
    ...
    if let Some(blockdevices) = json["blockdevices"].as_array() {
        for dev in blockdevices {
            ...
            let total_space = dev["size"].as_u64().or_else(|| dev["size"].as_str().and_then(|s| s.parse().ok())).unwrap_or(0);
            let used_space = dev["fsusage"].as_u64().or_else(|| dev["fsusage"].as_str().and_then(|s| s.parse().ok())).unwrap_or(0);
            let free_space = total_space.saturating_sub(used_space);
            let is_removable = dev["rm"].as_bool().unwrap_or(false) || dev["rm"].as_str() == Some("1");
            let filesystem = dev["fstype"].as_str().unwrap_or("unknown").to_string();

            // Core safety: Lock system partitions
            let is_locked = label.contains("SYSTEM") || label.contains("ROOT") || mount_point == "/";
...
pub async fn verify_security_action(pin: String, user_confirm: String) -> Result<bool, String> {
    println!("[AUTH] Verifying high-risk action for user: {}", user_confirm);
...
pub async fn request_boot_resize(name: String) -> Result<(), String> {
    println!("[STORAGE] Scheduling expansion for {} on next boot...", name);
    ...
    tauri::async_runtime::spawn(async move {
        ...
        loop {
            match get_disk_info().await {
                Ok(disks) => {
                    println!("[Magnolia Storage Pulse] Audit success: {} devices detected.", disks.len());
                    for disk in disks {
                        if disk.free_space < 1024 * 1024 * 512 { // 500MB warning
                            println!("[Magnolia Storage WARN] Low disk space on {}: {} bytes free.", disk.label, disk.free_space);
                        }
                    }
                },
